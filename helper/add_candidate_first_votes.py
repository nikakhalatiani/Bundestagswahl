...
try:
    ...
    candidates_df = pd.read_csv(
        input_candidates, delimiter=";", encoding="utf-8")
    wahlkreis_df = pd.read_csv(
        input_wahlkreis, delimiter=";", encoding="utf-8")
    ...
    wahlkreis_map = dict(
        zip(wahlkreis_df["Gebietsnummer"], wahlkreis_df["Gebietsname"]))

    # --- Prepare candidates: convert Wahlkreis to int (handling NaN) ---
    # First, fill NaN with -1, then convert to int
    candidates_df["Wahlkreis_int"] = candidates_df["Wahlkreis"].fillna(
        -1).astype(int)

    # --- Merge results with candidates ---
    # Match on: Wahlkreis (as Gebietsnummer) and party short name
    # results may use 'Gruppenname' as party column
    results_party_col = "Gruppenname" if "Gruppenname" in wahlkreis_results.columns else None
    if results_party_col is None:
        raise KeyError(
            "No party column found in results (expected 'Gruppenname')")

    merged = pd.merge(
        candidates_df,
        wahlkreis_results[["Gebietsnummer", results_party_col, "Anzahl"]],
        left_on=["Wahlkreis_int", "GruppennameKurz"],
        right_on=["Gebietsnummer", results_party_col],
        how="left"
    )

    # --- Clean up: remove helper columns and rename Anzahl ---
    merged.drop(columns=["Gebietsnummer", "Gruppenname",
                "Wahlkreis_int"], inplace=True, errors="ignore")
