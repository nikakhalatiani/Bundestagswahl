...
router.post('/ballot', async (req, res) => {
    ...
    try {
        ...
        const constituencyElectionId = Number(codeRes.rows[0].constituency_election_id);
        ...
        if (body.first && body.first.type === 'candidate' && body.first.person_id) {
            // Ensure the direct_candidacy exists for that person/constituency election
            const check = await pool.query(
                `SELECT 1 FROM direct_candidacy WHERE person_id = $1 AND constituency_election_id = $2`,
                [body.first.person_id, constituencyElectionId]
            );
            ...
            // Invalid first vote: pick any direct_candidacy person for constituency election to satisfy FK
            const pick = await pool.query(
                `SELECT person_id FROM direct_candidacy WHERE constituency_election_id = $1 LIMIT 1`,
        ...
        await pool.query(
            `INSERT INTO first_votes (direct_person_id, constituency_election_id, is_valid) VALUES ($1, $2, $3)`,
            [firstPersonId, constituencyElectionId, firstIsValid]
        );
        ...
        await pool.query(
            `INSERT INTO second_votes (party_list_id, constituency_election_id, is_valid) VALUES ($1, $2, $3)`,
            [partyListId, constituencyElectionId, secondIsValid]
        );
...
router.get('/constituency/:number/parties', async (req, res) => {
    ...
    try {
        let constituencyElection = null;
        if (constituencyElectionId) {
            constituencyElection = await resolveConstituencyElectionByBridgeId(constituencyElectionId);
        } else {
            constituencyElection = await resolveConstituencyElectionByNumber(constituencyNumber, year);
        }
        if (!constituencyElection) {
            return res.status(404).json({ error: 'constituency_not_found' });
        }
       ...
       ORDER BY p.short_name`,
            [constituencyElection.state_id, constituencyElection.year]
        );
    ...
    try {
        ...
        const result = await pool.query(
            `SELECT
             ...
             WHERE dc.constituency_election_id = $1
             ORDER BY p.short_name, per.last_name, per.first_name`,
            [constituencyElection.bridge_id]
        );
