DROP MATERIALIZED VIEW "public"."federal_distribution_cache";--> statement-breakpoint
DROP MATERIALIZED VIEW "public"."mv_constituency_party_votes";--> statement-breakpoint
DROP MATERIALIZED VIEW "public"."mv_constituency_stats";--> statement-breakpoint
DROP MATERIALIZED VIEW "public"."party_summary_cache";--> statement-breakpoint
DROP MATERIALIZED VIEW "public"."state_distribution_cache";--> statement-breakpoint
DROP MATERIALIZED VIEW "public"."seat_allocation_cache";--> statement-breakpoint
CREATE MATERIALIZED VIEW "public"."mv_constituency_first_votes" AS (
    SELECT
      dcv.constituency_id,
      dcv.year,
      dcv.party_id,
      COALESCE(SUM(dcv.first_votes), 0) AS first_votes
    FROM mv_direct_candidacy_votes dcv
    GROUP BY dcv.constituency_id, dcv.year, dcv.party_id
  ) WITH NO DATA;--> statement-breakpoint
CREATE MATERIALIZED VIEW "public"."mv_constituency_invalid_votes" AS (
    ...
    SELECT
      COALESCE(i1.constituency_id, i2.constituency_id) AS constituency_id,
      COALESCE(i1.year, i2.year) AS year,
      COALESCE(i1.invalid_first, 0) AS invalid_first,
      COALESCE(i2.invalid_second, 0) AS invalid_second
    FROM invalid_first i1
    FULL OUTER JOIN invalid_second i2
      ON i2.constituency_id = i1.constituency_id
     AND i2.year = i1.year
  ) WITH NO DATA;--> statement-breakpoint
CREATE MATERIALIZED VIEW "public"."mv_constituency_second_votes" AS (
    SELECT
      sv.constituency_id,
      pl.year,
      pl.party_id,
...
CREATE MATERIALIZED VIEW "public"."mv_constituency_vote_totals" AS (
    ...
    second_totals AS (
      SELECT
        constituency_id,
        year,
        COALESCE(SUM(second_votes), 0) AS valid_second
      FROM mv_constituency_second_votes
      GROUP BY constituency_id, year
    )
    SELECT
      COALESCE(ft.constituency_id, st.constituency_id) AS constituency_id,
      COALESCE(ft.year, st.year) AS year,
      COALESCE(ft.valid_first, 0) AS valid_first,
      COALESCE(st.valid_second, 0) AS valid_second
    FROM first_totals ft
    FULL OUTER JOIN second_totals st
      ON st.constituency_id = ft.constituency_id
     AND st.year = ft.year
  ) WITH NO DATA;--> statement-breakpoint
